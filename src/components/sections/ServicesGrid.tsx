...
import { Check, ArrowRight, Sparkles, X, ChevronRight, CheckCircle2, Clock, Wrench, ExternalLink } from 'lucide-react';
...
export const ServicesGrid: React.FC<ServicesGridProps> = ({ services }) => {
  ...
  return (
    <section id="services" className="py-24 relative z-10 bg-[#030712] border-t border-amber-500/20">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {activeServices.map((service) => (
            ...
            >
              ...
              {/* Action Trigger */}
              <div className="pt-6 mt-6 border-t border-slate-800 flex items-center justify-between flex-wrap gap-2">
                <button
                  type="button"
                  onClick={() => {
      ...
      {/* Interactive Service Scope Modal Popup with Portal */}
      {selectedService && typeof window !== 'undefined' && createPortal(
        <div
          className="fixed inset-0 z-[9999] bg-slate-950/85 backdrop-blur-sm flex items-center justify-center p-4 overflow-y-auto"
          onClick={() => setSelectedService(null)}
        >
          <div
            className="relative w-full max-w-2xl bg-[#0b1329] border border-amber-500/60 rounded-[2.5rem] p-6 sm:p-8 shadow-[0_0_60px_rgba(0,229,255,0.25)] space-y-6 max-h-[90vh] overflow-y-auto my-auto text-slate-100"
            onClick={(e) => e.stopPropagation()}
          >
            <button
              type="button"
              onClick={() => setSelectedService(null)}
              className="absolute top-6 right-6 p-2 rounded-full bg-slate-900 text-slate-400 hover:text-white border border-slate-700 hover:border-amber-400 transition-colors"
              aria-label="Close"
            >
              <X className="w-5 h-5" />
            </button>
            ...
            <div className="flex items-center gap-4 pr-8">
              <div className="w-14 h-14 rounded-2xl bg-slate-900 border border-amber-500/40 flex items-center justify-center text-amber-400 flex-shrink-0">
                  ...
                  {selectedService.features && selectedService.features.length > 0 && (
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5 pt-2">
                      {selectedService.features.map((feat, idx) => (
                        <div key={idx} className="flex items-center gap-2.5 text-xs font-semibold text-slate-200 bg-slate-900/90 p-3 rounded-xl border border-slate-800">
            ...
            <div className="flex items-center justify-between pt-4 border-t border-slate-800 flex-wrap gap-3">
              ...
              {selectedService.slug && (
                <Link
                  ...
                  onClick={() => setSelectedService(null)}
                  className="px-5 py-2.5 rounded-xl text-xs font-bold text-cyan-300 bg-slate-900 border border-cyan-500/40 hover:bg-slate-800 hover:text-white transition-colors inline-flex items-center gap-1.5"
                >
